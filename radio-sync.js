...
export function initAdminRadioSync(db, fb, auth) {
  ...
  // --- Login ADM (para servir de "fallback" quando index.html abre admin.html) ---
  // admin.html já tem login(), mas o painel pode ser carregado sem esse binding.
  // Aqui garantimos que os handlers existam.
  if (!window.login) {
    window.login = function () {
      try {
        ...
        // a implementação real fica no admin.html (signInWithEmailAndPassword)
        // Se não existir, só informa.
        if (typeof window.__admDoLogin === 'function') {
          window.__admDoLogin(email, senha);
        } else {
          console.warn('login() não foi injetado. Use admin.html para autenticação.');
          alert('Falha ao iniciar login ADM.');
        }
      } catch (e) {
        console.error(e);
        alert('Falha ao iniciar login ADM.');
